...
<body class="bg-gray-50 text-gray-800 h-screen overflow-hidden flex">
        ...
        <section class="max-w-4xl bg-white border border-gray-100 rounded-lg p-8 shadow-sm">
            <form action="{{ route('account.store') }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <!-- Input 4: Shift Dropdown — hanya tampil saat Role = User -->
                    <div id="shift-wrapper" class="{{ old('role') == 'User' ? '' : 'hidden' }} transition-all duration-300">
                        <label for="shift" class="block text-sm font-bold text-gray-700 mb-2">
                            Shift
                            <span class="text-[#8b0000] ml-0.5">*</span>
                        </label>
                        <div class="relative">
                        ...
                        <p class="text-xs text-gray-400 mt-1.5">Wajib diisi untuk akun dengan role User.</p>
                    </div>
    ...
    <!-- Inline Script for Password Visibility & Role/Shift Toggle -->
    <script>
        ...
        // Tampilkan / sembunyikan field Shift berdasarkan Role
        function handleRoleChange(role) {
            const shiftWrapper = document.getElementById('shift-wrapper');
            const shiftSelect  = document.getElementById('shift');

            if (role === 'User') {
                // Tampilkan field Shift dan jadikan required
                shiftWrapper.classList.remove('hidden');
                shiftSelect.setAttribute('required', 'required');
            } else {
                // Sembunyikan field Shift, hapus required, dan reset nilainya
        ...
        // Jalankan saat halaman load (untuk kondisi old() saat validasi gagal)
