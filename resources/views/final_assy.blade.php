...
<body class="bg-gray-50 text-gray-800 min-h-screen flex">
    ...
    <aside class="w-64 bg-white border-r border-gray-200 flex flex-col justify-between shrink-0">
        <div>
            ...
            <nav class="p-4 space-y-1">
                ...
                <a href="{{ route('final_assy.index') }}" class="flex items-center space-x-3 px-4 py-3 rounded-lg text-sm font-semibold bg-[#8b0000] text-white shadow-sm transition-all">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                ...
                <form id="filterForm" method="GET" action="{{ route('final_assy.index') }}" class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                        ...
                        <div class="relative min-w-[130px]">
                            <select name="line" onchange="this.form.submit()" class="w-full appearance-none pl-4 pr-10 py-2 border border-gray-200 rounded-xl text-xs font-semibold text-gray-600 bg-white focus:outline-none focus:ring-2 focus:ring-[#8b0000]/20 focus:border-[#8b0000] cursor-pointer">
                                <option value="all">Semua Line</option>
                                @foreach($lineOptions as $option)
                                    <option value="{{ $option }}" {{ (string)$selectedLine === (string)$option ? 'selected' : '' }}>Line {{ $option }}</option>
                                @endforeach
                            </select>
                            <div class="absolute inset-y-0 right-0 pr-3.5 flex items-center pointer-events-none text-gray-400">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        ...
                        @if($dateRange || ($selectedDefect && $selectedDefect !== 'all') || ($selectedLine && $selectedLine !== 'all'))
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="border-b border-gray-200">
                                ...
                                <th class="text-xs font-semibold text-gray-400 uppercase tracking-wider pb-3 pl-4">Line / Conveyor</th>
                        ...
                        <tbody>
                            @forelse($records as $record)
                                <tr class="border-b border-gray-100 last:border-b-0 hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td class="py-4 text-sm text-gray-950 font-bold pl-12">
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="py-12 text-center text-sm text-gray-400 font-medium">Tidak ada data defect untuk filter terpilih.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <script>
        ...
        function exportCSV() {
            ...
            csvContent += "Waktu,User,Jenis Assy,Line / Conveyor,Jenis Defect,Jenis Sub Defect,Quantity\n";
            ...
            link.setAttribute("download", "report_final_assy_" + new Date().toISOString().slice(0,10) + ".csv");
