...
@section('title', 'Report Pre Assy')

@section('content')
        <!-- Header Section -->
        <header class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-2xl font-black text-gray-900 tracking-tight leading-tight">Report Pre Assy</h1>
                <p class="text-xs font-semibold text-gray-500 mt-1">Riwayat aktivitas defect pre assy secara real-time.</p>
            </div>
                    ...
                    <table class="w-full min-w-[850px] text-left border-collapse">
                        <thead>
                            <tr class="border-b border-border bg-gray-50/70 sticky top-0 z-10">
                                ...
                                <th class="text-[11px] font-bold text-gray-400 uppercase tracking-wider py-3 px-4">No Terminal</th>
                                <th class="text-[11px] font-bold text-gray-400 uppercase tracking-wider py-3 px-4">No Mesin</th>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function fetchLivePreAssy() {
            const currentQuery = window.location.search;
            fetch('{{ url("/api/pre-assy/live") }}' + currentQuery)
                .then(r => r.json())
                .then(res => {
                ...
                .catch(err => console.error('[Polling] Gagal fetch pre assy live:', err));
        }

        // Init polling timer
        pollingTimer = setInterval(fetchLivePreAssy, 8000);
