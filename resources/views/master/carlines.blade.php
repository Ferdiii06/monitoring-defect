...
<head>
    ...
    <title>Master Carline / Konveyor - Sistem Monitoring Defect</title>
...
<body class="bg-gray-50 text-gray-800 h-screen overflow-hidden flex" x-data="{ editModal: false, editId: null, editCarTypeId: '', editName: '' }">
    ...
    <main class="flex-1 overflow-y-auto px-10 py-8 flex flex-col justify-start">
        ...
        <header class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 leading-tight">Master Carline / Konveyor</h1>
                <p class="text-sm text-gray-500 mt-1">Kelola data master line / conveyor berdasarkan jenis mobil.</p>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <section class="bg-white border border-gray-100 rounded-lg p-6 shadow-sm h-fit">
                <div class="flex items-center space-x-2 mb-4 pb-3 border-b border-gray-100">
                    ...
                    <h2 class="text-sm font-bold text-gray-900">Tambah Carline / Konveyor</h2>
                </div>

                <form action="{{ route('admin.master.carlines.store') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label for="car_type_id" class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-2">Jenis Mobil <span class="text-red-500">*</span></label>
                        <select id="car_type_id" name="car_type_id" required
                            ...
                            @foreach ($carTypes as $ct)
                                <option value="{{ $ct->id }}" {{ old('car_type_id', $selectedCarType) == $ct->id ? 'selected' : '' }}>{{ $ct->name }}</option>
                            @endforeach
                        </select>
                    </div>

                    <div>
                        <label for="name" class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-2">Nama Carline / Konveyor <span class="text-red-500">*</span></label>
                        <input type="text" id="name" name="name" placeholder="Contoh: LINE 1, CONVEYOR A" value="{{ old('name') }}" required
            ...
            <section class="lg:col-span-2 bg-white border border-gray-100 rounded-lg p-6 shadow-sm flex flex-col justify-between">
                <div>
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4 pb-3 border-b border-gray-100">
                        <h2 class="text-sm font-bold text-gray-900">Daftar Carline / Konveyor ({{ $carlines->total() }})</h2>
                        
                        <!-- Filter Jenis Mobil -->
                        <form method="GET" action="{{ route('admin.master.carlines.index') }}" class="flex items-center space-x-2">
                            <label for="filter_car_type_id" class="text-xs font-semibold text-gray-500 shrink-0">Filter:</label>
                            <select id="filter_car_type_id" name="car_type_id" onchange="this.form.submit()"
                                class="border border-gray-200 rounded-lg px-3 py-1.5 text-xs text-gray-700 focus:outline-none focus:ring-1 focus:ring-[#8b0000] bg-white">
                                <option value="">Semua Jenis Mobil</option>
                                @foreach ($carTypes as $ct)
                                    <option value="{{ $ct->id }}" {{ $selectedCarType == $ct->id ? 'selected' : '' }}>{{ $ct->name }}</option>
                                @endforeach
                            </select>
                            @if ($selectedCarType)
                                <a href="{{ route('admin.master.carlines.index') }}" class="text-xs text-gray-400 hover:text-red-700 ml-1">Reset</a>
                            @endif
                        </form>
                    </div>
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="border-b border-gray-200">
                                    ...
                                    <th class="text-xs font-semibold text-gray-400 uppercase tracking-wider pb-3 px-4">Carline / Konveyor</th>
                                    <th class="text-xs font-semibold text-gray-400 uppercase tracking-wider pb-3 px-4 text-center pr-2 w-32">Aksi</th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse ($carlines as $index => $carline)
                                    <tr class="border-b border-gray-100 last:border-b-0 hover:bg-gray-50/50 transition-colors">
                                        <td class="py-3.5 text-xs text-gray-500 px-4 pl-2 font-medium">
                                            {{ ($carlines->currentPage() - 1) * $carlines->perPage() + $loop->iteration }}
                                        </td>
                                        <td class="py-3.5 text-xs font-bold text-gray-800 px-4">
                                            <span class="inline-block bg-gray-100 text-gray-800 text-xs font-bold px-2.5 py-1 rounded-md tracking-wider">
                                                {{ $carline->carType->name ?? '-' }}
                                            </span>
                                        </td>
                                        <td class="py-3.5 text-sm font-semibold text-gray-900 px-4">
                                            {{ $carline->name }}
                                        </td>
                                        <td class="py-3.5 text-center px-4 pr-2">
                                            <div class="inline-flex items-center space-x-1.5">
                                                <!-- Tombol Edit -->
                                                <button type="button" 
                                                    @click="editModal = true; editId = {{ $carline->id }}; editCarTypeId = '{{ $carline->car_type_id }}'; editName = '{{ addslashes($carline->name) }}'"
                                                ...
                                                <!-- Tombol Hapus -->
                                                <form action="{{ route('admin.master.carlines.destroy', $carline->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus carline ini?');" class="inline-block">
                                                    @csrf
                                                    @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="py-10 text-center text-xs text-gray-400 font-medium">Belum ada data carline.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    <div x-show="editModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto bg-black/50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-gray-100" @click.away="editModal = false">
            <div class="flex justify-between items-center pb-3 border-b border-gray-100 mb-4">
                <h3 class="text-base font-bold text-gray-900">Edit Carline / Konveyor</h3>
                <button type="button" @click="editModal = false" class="text-gray-400 hover:text-gray-600">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                </button>
            </div>

            <form :action="'{{ url('/master/carlines') }}/' + editId" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                <div>
                    <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-2">Jenis Mobil <span class="text-red-500">*</span></label>
                    <select name="car_type_id" x-model="editCarTypeId" required
                        class="w-full border border-gray-200 rounded-lg p-3 text-sm focus:outline-none focus:ring-1 focus:ring-[#8b0000] focus:border-[#8b0000] bg-white">
                        <option value="">-- Pilih Jenis Mobil --</option>
                        @foreach ($carTypes as $ct)
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-2">Nama Carline / Konveyor <span class="text-red-500">*</span></label>
