...
<head>
    ...
    <title>Master Inspect Proses - Sistem Monitoring Defect</title>
...
<body class="bg-gray-50 text-gray-800 h-screen overflow-hidden flex" x-data="{ editModal: false, editId: null, editName: '' }">
    ...
    <main class="flex-1 overflow-y-auto px-10 py-8 flex flex-col justify-start">
        ...
        <header class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 leading-tight">Master Inspect Proses</h1>
                <p class="text-sm text-gray-500 mt-1">Kelola data master jenis proses inspeksi untuk Pre Assy.</p>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <section class="bg-white border border-gray-100 rounded-lg p-6 shadow-sm h-fit">
                <div class="flex items-center space-x-2 mb-4 pb-3 border-b border-gray-100">
                    ...
                    <h2 class="text-sm font-bold text-gray-900">Tambah Inspect Proses</h2>
                </div>

                <form action="{{ route('admin.master.inspect_process_types.store') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label for="name" class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-2">Nama Proses Inspect <span class="text-red-500">*</span></label>
                        <input type="text" id="name" name="name" placeholder="Contoh: SUB ASSY, VISUAL, DIMENSI" value="{{ old('name') }}" required
            ...
            <section class="lg:col-span-2 bg-white border border-gray-100 rounded-lg p-6 shadow-sm flex flex-col justify-between">
                <div>
                    <div class="flex justify-between items-center mb-4 pb-3 border-b border-gray-100">
                        <h2 class="text-sm font-bold text-gray-900">Daftar Inspect Proses ({{ count($processes) }})</h2>
                    </div>
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="border-b border-gray-200">
                                    ...
                                    <th class="text-xs font-semibold text-gray-400 uppercase tracking-wider pb-3 px-4">Nama Proses Inspect</th>
                            ...
                            <tbody>
                                    ...
                                    <tr class="border-b border-gray-100 last:border-b-0 hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td class="py-3.5 text-center px-4 pr-2">
                                            <div class="inline-flex items-center space-x-1.5">
                                                <!-- Tombol Edit -->
                                                <button type="button" 
                                                    @click="editModal = true; editId = {{ $process->id }}; editName = '{{ addslashes($process->name) }}'"
                                                ...
                                                <!-- Tombol Hapus -->
                                                <form action="{{ route('admin.master.inspect_process_types.destroy', $process->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus proses inspect ini?');" class="inline-block">
                                                    @csrf
                                                    @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="3" class="py-10 text-center text-xs text-gray-400 font-medium">Belum ada data jenis proses inspect.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    <div x-show="editModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto bg-black/50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-gray-100" @click.away="editModal = false">
            <div class="flex justify-between items-center pb-3 border-b border-gray-100 mb-4">
                <h3 class="text-base font-bold text-gray-900">Edit Inspect Proses</h3>
                <button type="button" @click="editModal = false" class="text-gray-400 hover:text-gray-600">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                </button>
            </div>

            <form :action="'{{ url('/master/inspect-process-types') }}/' + editId" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                <div>
                    <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-2">Nama Proses Inspect <span class="text-red-500">*</span></label>
