...
<head>
    ...
    <title>Master Jenis Mobil - Sistem Monitoring Defect</title>
...
<body class="bg-gray-50 text-gray-800 h-screen overflow-hidden flex" x-data="{ editModal: false, editId: null, editName: '' }">
    ...
    <main class="flex-1 overflow-y-auto px-10 py-8 flex flex-col justify-start">
        ...
        <header class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 leading-tight">Master Jenis Mobil</h1>
                <p class="text-sm text-gray-500 mt-1">Kelola data master jenis mobil (Toyota, Nissan, Mazda, dll).</p>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <section class="bg-white border border-gray-100 rounded-lg p-6 shadow-sm h-fit">
                <div class="flex items-center space-x-2 mb-4 pb-3 border-b border-gray-100">
                    ...
                    <h2 class="text-sm font-bold text-gray-900">Tambah Jenis Mobil</h2>
                </div>

                <form action="{{ route('admin.master.car_types.store') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label for="name" class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-2">Nama Jenis Mobil <span class="text-red-500">*</span></label>
                        <input type="text" id="name" name="name" placeholder="Contoh: TOYOTA, NISSAN, MAZDA" value="{{ old('name') }}" required
            ...
            <section class="lg:col-span-2 bg-white border border-gray-100 rounded-lg p-6 shadow-sm flex flex-col justify-between">
                <div>
                    <div class="flex justify-between items-center mb-4 pb-3 border-b border-gray-100">
                        <h2 class="text-sm font-bold text-gray-900">Daftar Jenis Mobil ({{ count($carTypes) }})</h2>
                    </div>
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="border-b border-gray-200">
                                    ...
                                    <th class="text-xs font-semibold text-gray-400 uppercase tracking-wider pb-3 px-4 text-center">Jumlah Carline</th>
                                    <th class="text-xs font-semibold text-gray-400 uppercase tracking-wider pb-3 px-4 text-center pr-2 w-32">Aksi</th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse ($carTypes as $index => $carType)
                                    <tr class="border-b border-gray-100 last:border-b-0 hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td class="py-3.5 text-xs text-gray-700 px-4 text-center font-semibold">
                                            <span class="bg-blue-50 text-blue-700 px-2.5 py-0.5 rounded-full font-bold">
                                                {{ $carType->carlines_count }} Carline
                                            </span>
                                        </td>
                                        <td class="py-3.5 text-center px-4 pr-2">
                                            <div class="inline-flex items-center space-x-1.5">
                                                <!-- Tombol Edit -->
                                                <button type="button" 
                                                    @click="editModal = true; editId = {{ $carType->id }}; editName = '{{ addslashes($carType->name) }}'"
                                                ...
                                                <!-- Tombol Hapus -->
                                                <form action="{{ route('admin.master.car_types.destroy', $carType->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus jenis mobil ini? Semua carline terkait juga akan terhapus.');" class="inline-block">
                                                    @csrf
                                                    @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="py-10 text-center text-xs text-gray-400 font-medium">Belum ada data jenis mobil.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    <div x-show="editModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto bg-black/50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-gray-100" @click.away="editModal = false">
            <div class="flex justify-between items-center pb-3 border-b border-gray-100 mb-4">
                <h3 class="text-base font-bold text-gray-900">Edit Jenis Mobil</h3>
                <button type="button" @click="editModal = false" class="text-gray-400 hover:text-gray-600">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                </button>
            </div>

            <form :action="'{{ url('/master/car-types') }}/' + editId" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                <div>
                    <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-2">Nama Jenis Mobil <span class="text-red-500">*</span></label>
